...
## if linux add -w0 to base64 command else do it the mac way
[[ "$OSTYPE" == "linux-gnu" ]] && WRAP="-w0"
echo args - $B64_ARGS $GOOGLE_APPLICATION_CREDENTIALS_FILE
export GOOGLE_APPLICATION_CREDENTIALS_SECRET=$(base64 ${WRAP} -i ${GOOGLE_APPLICATION_CREDENTIALS_FILE})
